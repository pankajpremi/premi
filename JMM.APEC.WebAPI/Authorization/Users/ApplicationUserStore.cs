...
namespace JMM.APEC.WebAPI.Infrastructure
{
    ...
    /// </summary>
    public class ApplicationUserStore<TUser, Id> : IUserLoginStore<TUser, int>,
        ...
        where TUser : ApplicationIdentityUser
    {
        ...
        private UserProfileTable profileTable;
        private RoleTable roleTable;
        private UserPortalTable userPortalTable;
        private UserRolesTable userRolesTable;
        private UserClaimsTable userClaimsTable;
        private UserLoginsTable userLoginsTable;
        public IdentityDatabase Database { get; private set; }
        ...
        //{
        //    new ApplicationUserStore<TUser>(new IdentityDatabase());
        //}

        public ApplicationUserStore(IdentityDatabase database, int portalId)
        {

            Database = database;

            userTable = new UserTable<TUser>(database);
            profileTable = new UserProfileTable(database);
        ...
        public Task CreateAsync(TUser user)
        {
            ...
            profileTable.Insert(user.Profile);

            int portalId = 0;

            if (user.Portals.Count > 0)
            {
                portalId = user.Portals[0].Id;
            }

            //assign user to portal
            userPortalTable.Insert(user.Id, portalId);
        ...
        public Task<int> IncrementAccessFailedCountAsync(TUser user)
        {
            ...
            var Enabled = Convert.ToBoolean(ConfigurationManager.AppSettings["UserLockoutEnabledByDefault"].ToString());
            var FailedCount = Convert.ToInt32(ConfigurationManager.AppSettings["MaxFailedAccessAttemptsBeforeLockout"].ToString());

            if (Enabled)
            {
                if (user.AccessFailedCount >= FailedCount)
                {
